...
import { CollectionViewer, DataSource, SelectionModel } from "./datasource";
import { EntityBooleanFunction } from "@app/modules/home/models/entity/entities-table-config.models";
import { AlarmEscalationFilter } from "@models/alarm-escalation-filter.models";
...
export class EntitiesDataSource<T extends BaseData<HasId>, P extends PageLink = PageLink> implements DataSource<T> {
    ...
    loadEntities(pageLink: P, alarmEscalationFilter?: AlarmEscalationFilter, alarmEscalationAuthority?: AuthUser): Observable<PageData<T>> {
        ...
        this.fetchFunction(pageLink).pipe(
            ...
            (pageData) => {
                if (alarmEscalationAuthority) {
                    if (alarmEscalationAuthority.authority == Authority.CUSTOMER_ADMIN || alarmEscalationAuthority.authority == Authority.CUSTOMER_USER) {
                        pageData.data = pageData.data.filter(x => Array.from(x['customerIds']).includes(alarmEscalationAuthority.customerId))
                        pageData.totalElements = pageData.data.length;
                    }
                }
                if (alarmEscalationFilter) {
                    if (alarmEscalationFilter.tiers !== 0) {
                        pageData.data = pageData.data.filter(x => x['tiers'] == alarmEscalationFilter.tiers)
                        pageData.totalElements = pageData.data.length;
                    }
                    if (alarmEscalationFilter.customerIds.length !== 0) {
                        pageData.data = pageData.data.filter(x =>
                            Array.from(x['customerIds']).some(customerId => alarmEscalationFilter.customerIds.includes(customerId as string))
                        )
                        pageData.totalElements = pageData.data.length;
                    }
                }
