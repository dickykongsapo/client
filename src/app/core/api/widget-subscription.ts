...
import { createLabelFromDatasource, deepClone, isDefined, isDefinedAndNotNull, isEqual } from '../utils';
import { EntityId } from '@models/id/entity-id';
import moment_ from 'moment';
import { emptyPageData, PageData } from '@models/page/page-data';
import { EntityDataListener } from '../services/entity-data.service';
import {
    AlarmData,
    AlarmDataPageLink,
    EntityData,
...
export class WidgetSubscription implements IWidgetSubscription {
    ...
    totalAlarmDataListener: AlarmDataListener;
    ...
    constructor(subscriptionContext: WidgetSubscriptionContext, public options: WidgetSubscriptionOptions, customerList?: Array<string>) {
        ...
        if (this.type === widgetType.rpc) {
        ...
        } else if (this.type === widgetType.alarm || this.type === widgetType.alarmCounter || this.type === widgetType.deviceHealthCounter) {
            ...
            this.totalAlarmDataListener = null;
            this.alarms = emptyPageData();
            this.totalAlarms = emptyPageData();
        ...
        // else if (this.type === widgetType.deviceHealthCounter) {
        //   this.callbacks.rpcStateChanged = this.callbacks.rpcStateChanged || (() => { });
        //   this.callbacks.onRpcSuccess = this.callbacks.onRpcSuccess || (() => { });
        //   this.callbacks.onRpcFailed = this.callbacks.onRpcFailed || (() => { });
        //   this.callbacks.onRpcErrorCleared = this.callbacks.onRpcErrorCleared || (() => { });
        ...
        //   this.loadingData = false;
        //   this.displayLegend = false;
        //   this.initDeviceHealthCounterSubscription().subscribe(() => {
        //     subscriptionSubject.next(this);
        //     subscriptionSubject.complete();
        //   },
        //     () => {
    ...
    private configureAlarmCounterData() {
        this.notifyDataLoaded();
    }
    ...
    onFiltersChanged(filterIds: Array<string>): boolean {
        if (this.type !== widgetType.rpc) {
            if (this.type === widgetType.alarm || this.type === widgetType.alarmCounter || this.type === widgetType.deviceHealthCounter) {
    ...
    onDashboardTimewindowChanged(newDashboardTimewindow: Timewindow) {
        if (this.type === widgetType.timeseries || this.type === widgetType.alarm || this.type === widgetType.alarmCounter || this.type === widgetType.deviceHealthCounter) {
            if (this.useDashboardTimewindow) {
    ...
    sendCommand(oneWayElseTwoWay: boolean, method: string, params?: any, timeout?: number): Observable<any> {
        if (!this.rpcEnabled) {
            ...
            const rpcSubject: Subject<void> = new ReplaySubject<void>();
    ...
    private extractRejectionErrorText(rejection) {
        let error = null;
        if (rejection.error) {
    ...
    update(isTimewindowTypeChanged = false) {
        if (this.type !== widgetType.rpc) {
            if (this.type === widgetType.alarm || this.type === widgetType.alarmCounter || this.type === widgetType.deviceHealthCounter) {
                ...
                this.updateTotalAlarmDataSubscription();
            } else {
    ...
    updateCustomerList(customerList: Array<string>, isTimewindowTypeChanged = false) {
        this.customerList = customerList;
    ...
    subscribeForTotalAlarms(pageLink: AlarmDataPageLink,
        keyFilters: KeyFilter[]) {
        if (this.totalAlarmDataListener) {
            this.ctx.alarmDataService.stopSubscription(this.totalAlarmDataListener);
        }
        ...
        this.totalAlarmDataListener = {
            subscriptionTimewindow: this.subscriptionTimewindow,
            alarmSource: this.alarmSource,
            alarmsLoaded: this.totalAlarmsLoaded.bind(this),
            alarmsUpdated: this.totalAlarmsUpdated.bind(this)
        };

        this.totalAlarms = emptyPageData();
        this.ctx.alarmDataService.subscribeForAlarms(this.totalAlarmDataListener, pageLink, keyFilters);
    ...
    private doSubscribe() {
        if (this.type !== widgetType.rpc && this.type !== widgetType.alarm && this.type !== widgetType.alarmCounter && this.type !== widgetType.deviceHealthCounter) {
            this.dataSubscribe();
        }
    }
    ...
    private totalAlarmsUnsubscribe() {
        if (this.totalAlarmDataListener) {
            this.ctx.alarmDataService.stopSubscription(this.totalAlarmDataListener);
            this.totalAlarmDataListener = null;
        }
    ...
    private checkAlarmCountSource(aliasIds: Array<string>): boolean {
        if (this.options.alarmSource && this.options.alarmSource.entityAliasId) {
            if (aliasIds.indexOf(this.options.alarmSource.entityAliasId) > -1) {
                this.updateAlarmSubscription();
            }
    ...
    private updateTotalAlarmDataSubscription() {
        if (this.totalAlarmDataListener) {
            const pageLink = this.totalAlarmDataListener.alarmDataSubscriptionOptions.pageLink;
            const keyFilters = this.totalAlarmDataListener.alarmDataSubscriptionOptions.additionalKeyFilters;
            this.subscribeForTotalAlarms(pageLink, keyFilters);
        }
    }
    ...
    private updateTimewindow() {
        ...
        if (this.subscriptionTimewindow.realtimeWindowMs) {
            if (this.subscriptionTimewindow.quickInterval) {
                ...
                this.timeWindow.maxTime = moment_().valueOf() + this.subscriptionTimewindow.tsOffset + this.timeWindow.stDiff;
    ...
    private dataLoaded(pageData: PageData<EntityData>,
        ...
        if (datasource.type === DatasourceType.entity &&
            ...
            if (this.warnOnPageDataOverflow) {
                const message = i18n.t('widget.data-overflow',
        ...
        return datasource.dataKeys.map((dataKey, keyIndex) => {
            ...
            if (this.comparisonEnabled && dataKey.isAdditional && dataKey.settings.comparisonSettings.comparisonValuesLabel) {
                ...
                if (this.comparisonEnabled && dataKey.isAdditional) {
                    dataKey.label = dataKey.label + ' ' + i18n.t('legend.comparison-time-ago.' + this.timeForComparison);
                }
    ...
    private alarmsLoaded(alarms: PageData<AlarmData>, allowedEntities: number, totalEntities: number) {
        ...
        if (totalEntities > allowedEntities) {
            const message = i18n.t('widget.alarm-data-overflow',
    ...
    private totalAlarmsLoaded(alarms: PageData<AlarmData>, allowedEntities: number, totalEntities: number) {
        this.totalAlarms = alarms;
    ...
    private totalAlarmsUpdated(updated: Array<AlarmData>, alarms: PageData<AlarmData>) {
        this.totalAlarmsLoaded(alarms, 0, 0);
    }
