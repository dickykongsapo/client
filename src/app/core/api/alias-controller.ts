...
import { Datasource, DatasourceType, datasourceTypeTranslationMap } from '@models/widget.models';
import { deepClone, isEqual } from '../utils';
import { EntityService } from '../services/entity.service';
import { AlarmService } from '../services/alarm.service';
import { UtilsService } from '../services/utils.service';
import { AliasFilterType, EntityAliases, SingleEntityFilter } from '@models/alias.models';
...
export class AliasController implements IAliasController {
                ...
                return this.getAliasInfo(newDatasource.entityAliasId).pipe(
                    mergeMap((aliasInfo) => {
                        ...
                        if (aliasInfo.resolveMultiple) {
                            ...
                            if (forceFilter) {
                                return this.entityService.findSingleEntityInfoByEntityFilter(aliasInfo.entityFilter).pipe(
                                    map((entity) => {
                                        if (entity) {
    ...
    resolveAlarmCountSource(): Observable<JSON> {
        return this.alarmService.getAlarmCount().pipe(
            map(alarmcount => {
                return alarmcount;
            })
        )
                ...
                result.forEach((datasource) => {
                    if (datasource.type === DatasourceType.function || datasource.type === DatasourceType.entityCount) {
                        ...
                        if (datasource.name && datasource.name.length) {
                            ...
                            name = (datasourceTypeTranslationMap.get(datasource.type));
